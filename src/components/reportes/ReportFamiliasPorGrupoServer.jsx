...
import FamiliasPorGrupoPDF from "@/components/reportes/FamiliasPorGrupoPDF";
import { useSearchParams } from "next/navigation";

const ReportFamiliasPorGrupoServer = () => {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          ...
          { params: { grupo, familia } }
        );
        setFamilias(response.data.familias || []);
        setConteo(response.data.conteo || []);
        setTotalAnimales(response.data.totalAnimales || 0);
      } catch (err) {
        console.error("Error fetching familias por grupo:", err);
        let msg = "Ocurrió un error al cargar el reporte de familias por grupo.";
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
            Reporte de Familias por Grupo
          </h1>
          <p className="text-gray-500 text-sm mt-1">
            Grupo:{" "}
            <span className="font-semibold text-primary">{grupo}</span>
            {familia !== "Todos" && (
              <>
                {" "}
                | Familia:{" "}
        ...
        <PDFDownloadLink
          ...
          fileName={`Reporte_FamiliasPorGrupo_${grupo}_${familia}_${new Date()
            .toISOString()
            .slice(0, 10)}.pdf`}
...
export default ReportFamiliasPorGrupoServer;